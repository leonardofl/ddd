...
Este documento é um resumo do livro ``\ddd: Atacando as Complexidades no Coração do Software''~\cite{Evans2003DDD}. Os objetivos deste resumo são:
...
Obs: \citacao{trechos nesta formatação são citações diretas do livro}. Conceitos chaves do livro em \emph{itálico}. Não há separação rígida entre oq o livro diz e oq eu digo, mas maioria das afirmativas são baseadas no livro. Explicitar as duas coisas quebraria a fluidez do texto. Este não é um texto acadêmico.
